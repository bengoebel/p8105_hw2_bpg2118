...
affiliations at the state and federal level. The data set has 
`r nrow(pols_month)` rows and `r ncol(pols_month)` columns. It contains 
political data from `r min(pull(pols_month, year))` to 
`r max(pull(pols_month, year))`. The key variables indicate the number of
Republicans and Democrats who are governors, representatives and senators. The
data set also has a key variable for the political affiliation of the
president. In the time frame of this data set, there has been a Republican
president for `r nrow(filter(pols_month, president == "gop"))` months, and
...
are `r nrow(snp)` rows and `r ncol(snp)` columns. Data is collect from
`r min(pull(snp, year))` to `r max(pull(snp, year))`. This time span is similar
to that of the pols-month data except that data collection started three years
...
are `r nrow(unemployment)` rows and `r ncol(unemployment)` columns. Data is collect from 
`r min(pull(unemployment, year))` to `r max(pull(unemployment, year))`. This 
time span is similar to that of the pols-month data except that data collection started one year later for the unemployment data than it did for the 
pols-month data. The key variable in this data set is unemployment_pct, which 
is the unemployment percentage on the observed date. The median unemployment
percentage is 
`r round(median(pull(unemployment, unemployment_pct), na.rm = TRUE), 2)`%.
...
popular_baby_names %>%
  ...
  pivot_wider(names_from = "year_of_birth",
              values_from = "count") %>%
  select(ethnicity, childs_first_name, `2011`:`2016`) %>%
...
popular_baby_names %>%
  ...
  knitr::kable(caption = "Most popular among male children from 2011 to 2016.")
```

Finally, for male, white non-hispanic children born in 2016, we will produce a 
scatter plot visualizing the number of children with a name (y axis) against 
...
popular_baby_names %>%
  ...
  ggplot(aes(x = rank, y = count)) +
    geom_point() +
  labs(
    title = "Number of Children with a Name as a Function of Rank in Popularity
             of that Name among Male, White Non-Hispanic Children Born in 2016",
